...
export default function Games() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setCategories({ gamesrow: data.category.gamesrow, gamesGroup: data.category.gamesGroup });
    })();
  }, []);
  const groups = categories?.gamesGroup?.map((e, index) => (
    <AppsGroup key={index + "33"} title={e.title} bg={e.bg} info={e.info} images={e.img} />
  ));
  const rows = categories?.gamesrow?.map((e, index) => (
